...
/**
 ...
 GaYaTextField is a class that represents a component from the design system.
 ...
 This GaYaTextField has 4 types:
 - Text (default type)
 - Name
 - Number
 - Password
 ...
 The GaYaTextField has 3 states for feedback, that can be configured with a message:
 - Error
 - Success
 ...
 The GaYaTextField also has an action item on the right, which can be configured with an icon from NatDSIcons, a local image or a remote image:
 ...
 To manage the GaYaTextField, use UITextFieldDelegate protocol as usual.
...
public class GaYaTextField: UIView {
    ...
    /// The type of the textfield, chosen from the `GaYaTextFieldType` enum
    public var type: GaYaTextFieldType = .text {
        didSet {
            handleTextFieldType()
        }
    }
    ...
    /// The size of the textfield, chosen from the `GaYaTextField.Size` enum
    ...
    public private(set) lazy var textField: GaYaField = {
        let field = GaYaField(theme: self.theme)
    ...
    public init(theme:AvailableTheme = .none) {
        self.theme = theme
        GaYaTextField.currentTheme = self.theme
...
public extension GaYaTextField {
...
extension GaYaTextField {
    ...
    private func addArrowView() {
        ...
        let trailingPadding: CGFloat = getComponentAttributeFromTheme(\.isInputRounded) ? -NatSpacing.small : -NatSpacing.tiny

        NSLayoutConstraint.activate([
            arrowView.centerYAnchor.constraint(equalTo: textField.centerYAnchor),
            arrowView.trailingAnchor.constraint(equalTo: trailingAnchor, constant: trailingPadding)
        ])
    }
...
extension GaYaTextField {
    ...
    /// - Parameter size: An option from `GaYaTextField.Size` enum
    public func configure(size: Size) {
        self.size = size
        
        if getComponentAttributeFromTheme(\.isButtonRounded) {
            layer.cornerRadius = self.size.value / 2
        } else {
            layer.cornerRadius = getComponentAttributeFromTheme(\.buttonBorderRadius)
        }
        
    }

    /// Sets the textField type, which changes it's configuration for keyboard, secure text, capitalization and autocorrection
    /// - Parameter type: An option from `GaYaTextFieldType`
    public func configure(type: GaYaTextFieldType) {
        self.type = type
    }

    /// Sets the state of the textField and a text to describe the state
    /// - Parameters:
    ///   - state: An option from `GaYaTextField.FeedbackState`: error, success or none
    ...
    /// Sets a delegate for the GaYaTextField
