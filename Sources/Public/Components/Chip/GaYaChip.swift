...
public final class GaYaChip: UIView {

    // MARK: - Private properties

    private let size: GaYaChip.Size
    ...
    public init(size: GaYaChip.Size = .semi,
                color: GaYaChip.Color = .neutral,
    ...
    private func removeFromStack(on position: GaYaChip.Position) {
    ...
    private func addToStack(view: UIView, on position: GaYaChip.Position) {
    ...
    /// Example of usage:
    /// ```
    /// gayaChip.configure(text: "GaYaChip example text")
    /// ```
    ...
    /// Example of usage:
    /// ```
    /// gayaChip.configure(icon: getIcon(.outlinedDefaultMockup), position: .left)
    /// ```
    /// - Parameters:
    ///   - icon:  A `String` that set an icon to the component
    ///   - position: A `GaYaChip.Position` that indicates the position of the icon
    public func configure(icon: String?, position: GaYaChip.Position) {
    ...
    /// Example of usage:
    /// ```
    /// gayaChip.configure(avatar: gayaAvatar, position: .left)
    /// ```
    /// - Parameters:
    ///   - avatar:  A `GaYaAvatar` that set an avatar to the component
    ///   - position: A `GaYaChip.Position` that indicates the position of the avatar
    public func configure(avatar: GaYaAvatar, position: GaYaChip.Position) {
    ...
    /// Example of usage:
    /// ```
    /// gayaChip.configure(state: .normal)
    /// ```
    ...
    /// Example of usage:
    /// ```
    /// gayaChip.configure { isSelected in }
    /// ```
    ...
    public func configure(color: Color) {
        switch color {
        case .primary:
            self.color = ThemeColor(theme: GaYaChip.currentTheme, colorType: .primary)
        case .onPrimary:
            self.color = ThemeColor(theme: GaYaChip.currentTheme, colorType: .onPrimary)
        case .secondary:
            self.color = ThemeColor(theme: GaYaChip.currentTheme, colorType: .secondary)
        case .onSecondary:
            self.color = ThemeColor(theme: GaYaChip.currentTheme, colorType: .onSecondary)
        case .neutral:
            self.color = ThemeColor(theme: GaYaChip.currentTheme, colorType: .neutral)
        case .inverse:
            self.color = ThemeColor(theme: GaYaChip.currentTheme, colorType: .inverse)
